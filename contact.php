<?php include 'header.php'; ?>
<link rel="stylesheet" href="css/contact.css">

<!-- ===== HERO SECTION ===== -->
<section class="hero">
  <div class="hero-text-box">
    <h1>Meet Our Team</h1>
    <p>The passionate individuals behind WaaWeeWoo Bookstore, dedicated to bringing you the best reading experience.</p>
  </div>
</section>

<!-- ===== CONTACT INFO SECTION ===== -->
<section class="contact-info">
  <div class="container">
    <div class="contact-card">
      <div class="contact-icon">📍</div>
      <h3>Visit Us</h3>
      <p>123 Book Street<br>Reading City, RC 12345<br>Malaysia</p>
    </div>
    <div class="contact-card">
      <div class="contact-icon">📞</div>
      <h3>Call Us</h3>
      <p>Phone: +60 12-785-0310<br>Business Hours: Mon-Fri 9AM-6PM</p>
    </div>
    <div class="contact-card">
      <div class="contact-icon">✉️</div>
      <h3>Email Us</h3>
      <p>General: info@waaweewoo.com<br>Support: support@waaweewoo.com</p>
    </div>
  </div>
</section>

<!-- ===== TEAM SECTION ===== -->
<section class="team-section">
  <div class="container">
    <h2>Our Team Members</h2>
    <div class="team">
      <div class="member">
        <div class="member-image">
          <img src="images/team-images/Formal-irfanshah1.jpg" alt="Muhammad Irfan bin Mohd Shahrizan">
          <div class="member-role">Project Lead</div>
        </div>
        <div class="member-content">
          <div class="member-info">
            <h3>Muhammad Irfan bin Mohd Shahrizan</h3>
            <p class="position">Project Lead & Database Architect</p>
            <p><strong>Responsibilities:</strong> Project management, backend development, database integration</p>
            <p><strong>Student ID:</strong> CBS24070258</p>
          </div>
          <div class="member-contact">
            <div class="contact-item">
              <i>📞</i>
              <span>012-7850310</span>
            </div>
          </div>
        </div>
      </div>

      <div class="member">
        <div class="member-image">
          <img src="images/user.png" alt="Adeeb">
          <div class="member-role">Admin</div>
        </div>
        <div class="member-content">
          <div class="member-info">
            <h3>Adeeb</h3>
            <p class="position">Admin - Book & Category Manager</p>
            <p><strong>Responsibilities:</strong> Managing books and categories, admin panel development</p>
            <p><strong>Student ID:</strong> CBS24070259</p>
          </div>
          <div class="member-contact">
            <div class="contact-item">
              <i>📞</i>
              <span>012-3456789</span>
            </div>
          </div>
        </div>
      </div>

      <div class="member">
        <div class="member-image">
          <img src="images/user.png" alt="Faheem">
          <div class="member-role">Admin</div>
        </div>
        <div class="member-content">
          <div class="member-info">
            <h3>Faheem</h3>
            <p class="position">Admin - User & Order Oversight</p>
            <p><strong>Responsibilities:</strong> User management, order processing, admin oversight</p>
            <p><strong>Student ID:</strong> CBS24070260</p>
          </div>
          <div class="member-contact">
            <div class="contact-item">
              <i>📞</i>
              <span>013-4567890</span>
            </div>
          </div>
        </div>
      </div>

      <div class="member">
        <div class="member-image">
          <img src="images/user.png" alt="Amar">
          <div class="member-role">Member</div>
        </div>
        <div class="member-content">
          <div class="member-info">
            <h3>Amar</h3>
            <p class="position">Member - Shopping Experience</p>
            <p><strong>Responsibilities:</strong> Shopping cart, product browsing, user experience</p>
            <p><strong>Student ID:</strong> CBS24070261</p>
          </div>
          <div class="member-contact">
            <div class="contact-item">
              <i>📞</i>
              <span>014-5678901</span>
            </div>
          </div>
        </div>
      </div>

      <div class="member">
        <div class="member-image">
          <img src="images/user.png" alt="Jazlan">
          <div class="member-role">Member</div>
        </div>
        <div class="member-content">
          <div class="member-info">
            <h3>Jazlan</h3>
            <p class="position">Member - Checkout & Profile Mgmt</p>
            <p><strong>Responsibilities:</strong> Checkout process, user profiles, payment integration</p>
            <p><strong>Student ID:</strong> CBS24070262</p>
          </div>
          <div class="member-contact">
            <div class="contact-item">
              <i>📞</i>
              <span>015-6789012</span>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>

<?php include 'footer.php'; ?>
